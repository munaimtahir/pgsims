...
private data class WorkflowPending(val title: String, val count: Int?)
    ...
    Scaffold(
        ...
        bottomBar = {
            NavigationBar {
                SupervisorDestination.entries.forEach { item ->
                    NavigationBarItem(
                        selected = destination == item && selectedResidentId == null,
                        onClick = { destination = item; selectedResidentId = null },
        ...
        Column(
            ...
            if (residentId != null) {
                ...
                SupervisorDestination.HOME -> SupervisorHomeContent(data)
...
@Composable
private fun SupervisorHomeContent(data: InstitutionalSnapshot) {
    val me = data.me
    ...
    val workflows = listOf(
        WorkflowPending("Logbook", dashboard?.number("pending_logbook_reviews_count")),
        WorkflowPending("Research / Synopsis", pending?.number("research_approvals")),
        WorkflowPending("Leave Requests", pending?.number("leave_approvals")),
        WorkflowPending("Rotations", pending?.number("rotation_approvals")),
        WorkflowPending("Evaluations", dashboard?.number("pending_evaluation_reviews_count")),
    )
    ...
    Text(
        "Approve, reject and revision actions are not yet available on Android — use PGR SIMS on the web " +
            "to act on pending items. This screen shows the same counts.",
        style = MaterialTheme.typography.bodySmall,
        color = MaterialTheme.colorScheme.onSurfaceVariant,
    )
}
