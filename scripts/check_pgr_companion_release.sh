...
# The single published pk.vexel.pgrcompanion listing is built from android/app-companion.
# at versionCode 7 / 1.1.7. This is an explicit repository-level decision — see
# builds/companion/build-info.json and SPRINT_STATE.md.
build="$root/android/app-companion/build.gradle.kts"
manifest="$root/android/app-companion/src/main/AndroidManifest.xml"
strings="$root/android/app-companion/src/main/res/values/strings.xml"
grep -q 'applicationId = "pk.vexel.pgrcompanion"' "$build"
grep -q 'versionCode = 7' "$build"
grep -q 'versionName = "1.1.7"' "$build"
...
if command -v rg >/dev/null 2>&1; then
    if rg -n -i "$forbidden" "$root/android/app-companion/src/main" >/dev/null; then
        echo "Forbidden release pattern found in PGR Companion main source." >&2
        exit 1
    fi
else
    if grep -RniE "$forbidden" "$root/android/app-companion/src/main" >/dev/null; then
        echo "Forbidden release pattern found in PGR Companion main source." >&2
        exit 1
    fi
fi
