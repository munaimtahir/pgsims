...
docker compose -p "$COMPOSE_PROJECT_NAME" stop frontend web worker beat || true

# Remove containers if they exist
echo "Removing containers..."
docker compose -p "$COMPOSE_PROJECT_NAME" rm -f frontend web worker beat || true

# Remove existing images to force rebuild (scoped to pgsims project)
echo "Removing existing images..."
docker compose -p "$COMPOSE_PROJECT_NAME" rmi -f frontend web worker beat || true
...
docker compose -p "$COMPOSE_PROJECT_NAME" build --no-cache frontend web worker beat

# Start all services
echo "Starting frontend, web, worker, and beat services..."
docker compose -p "$COMPOSE_PROJECT_NAME" up -d frontend web worker beat
...
docker compose -p "$COMPOSE_PROJECT_NAME" ps frontend web worker beat

echo "=========================================="
echo "Services are now running:"
echo "  Frontend: http://127.0.0.1:8082 (internal)"
echo "  Backend:  http://127.0.0.1:8014 (internal)"
echo "  Public:   https://pgsims.alshifalab.pk"
echo "  API:      https://pgsims.alshifalab.pk/api/"
