...
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" stop web || true

# Remove the web container if it exists
echo "Removing web container..."
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" rm -f web || true

# Remove the web image to force rebuild (scoped to pgsims project)
echo "Removing existing web image..."
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" rmi -f web || true
...
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" build --no-cache web

# Start the web service
echo "Starting web service..."
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" up -d web
...
docker compose -f ../../docker/docker-compose.yml -p "$COMPOSE_PROJECT_NAME" ps web
