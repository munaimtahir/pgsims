...
| ANY | `api/^rotations/(?P<pk>[^/.]+)/supervisor-approve/$` | RotationAssignmentViewSet | func | None | None |
...
- **ANY api/^rotations/(?P<pk>[^/.]+)/supervisor-approve/$** -> lib/api/training.ts (POST), lib/api/training.ts (GET)
