...
**Repository-level decision (2026-09-20): the `pk.vexel.pgrcompanion` Play listing is built
from `android/app-companion` — the login-gated postgraduate management client — and the current
candidate is `versionCode 9` / `1.1.9`. It supersedes the prior offline-only and historical
1.1.3 builds.
...
- Version: `1.1.9` (versionCode `9`)
- Source module: `android/app-companion`
- Artifact: existing verified candidate under `builds/companion/1.1.9-parity-main/`; no new bundle
  is built during the current gap-closure sprint.
...
- [ ] Confirm versionCode `9` has not already been uploaded for this listing.
...
- [ ] Upload only the final frozen AAB after gap-closure acceptance; compare its recorded SHA-256
      before selecting it in Play Console. The current sprint intentionally does not rebuild or upload.
