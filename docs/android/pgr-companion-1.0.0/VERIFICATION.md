...
This app has no native libraries, so no native debug-symbols ZIP is required. The R8 mapping file is provided separately as `pgr-companion-1.0.0-r8-mapping.txt` and belongs in Play Console's deobfuscation/R8 mapping upload field, not the native symbols field. Do not upload a ZIP containing `mapping.txt`, `configuration.txt`, or `resources.txt` as native symbols.
