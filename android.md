...
| [ ] Complete pagination/filter/search/error recovery for lists | All permitted roles | directory, academic list/report and bulk components | `Each list endpoint with server query parameters/next/count` | R.paged unwraps one page; no next-page loop | B | Q | 5 | List contracts; >page-size data, stable ordering and filters give complete results without false empty/error states |
