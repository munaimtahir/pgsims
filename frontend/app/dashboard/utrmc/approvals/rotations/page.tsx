...
function StatusBadge({ status }: { status: string }) {
  ...
  return <span className={`text-xs font-medium px-2 py-0.5 rounded ${colors[status] ?? 'bg-gray-100 text-gray-700'}`}>{status}</span>;
}
...
export default function RotationApprovalsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await apiClient.get('/api/utrmc/approvals/rotations/');
      setItems(res.data.results ?? res.data);
    } catch { setError('Failed to load rotation approvals.'); }
      ...
      <DashboardLayout>
        <div className="space-y-6">
          <div>
            ...
            <p className="mt-1 text-gray-500">Pending rotation assignments awaiting review.</p>
          </div>
          {error && <ErrorBanner message={error} />}
          <SectionCard title={`Pending Approvals (${items.length})`}>
